---
name: global-sync
description: 可将「Agent 已实现的能力」写清传入，也可零输入；零输入时由 Agent 根据当前上下文推断用户与项目关心的能力，先输出知识库更新大纲、确认后写入 Rules/Skills/索引以注入上下文；触发：global-sync、全局同步、知识库同步、已实现能力
---
> **「配置根」**：当前 agent 对应的 AI 工具配置目录（`flow2spec init` 写入，常见 **`.cursor/`**、**`.claude/`**、**`.codex/`**）。下文 **`配置根/...`** 指该目录下的相对路径。

# global-sync（知识库同步 · 先大纲后写入）

在用户要执行 **global-sync** 时：把**用户本次描述**、**当前 Agent / 会话角色相关描述**、**与项目相关的功能与约定**、以及**要注入上下文的「能力」集合**（见下文「输入」）整理为对**知识库**的更新方案；**必须先输出「更新大纲」并征得用户明确确认**，再写入 **`rules/`**、**`skills/`**（在既有结构上**增补或修订**，不无故整目录删除模板技能）、**`docs-index.md`**、**`main.mdc`** 等，使这些能力在后续对话中可被 **Rule / Skill / 索引** 加载，即**注入到 Agent 可用上下文**。可选在 **`stock-docs/`** 增加简短摘录类 MD 作为索源（若需要且大纲已列明）。
...
4. **拟变更清单**（逐项、可到路径级）：
   ...
   - **stock-docs**（可选）：是否新增摘录类 MD、文件名建议、是否后续再走 **generate-project-context**（若需从终稿生成大套 Rules/Skills，应在大纲中单独列为可选阶段）。
...
- 写入 **`rules/`**、**`skills/`** 时遵守项目既有风格与 frontmatter；若涉及从 **`stock-docs/`** 链出或写 **sourceDoc**，遵守 **`generate-project-context`** 技能中的「文档路径与链接约定」（参见 `配置根/skills/generate-project-context/SKILL.md`）。
- 若大纲列有「先 **spec2context-md** / **generate-project-context**」等后续技能：在本技能完成**已确认**的写入后，再引导用户按需触发对应技能，**不要**在本技能内跳过确认擅自执行长链路。
...
- **与 global-fix / global-feat 区分**：本技能侧重「会话与项目现状 → 知识库对齐」；针对**已指明的规则违反**用 **global-fix**；针对**新增能力**用 **global-feat**。
