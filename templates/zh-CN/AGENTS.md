...
由 **`flow2spec init`** 将 npm 包内 **`templates/<locale>/rules/*.mdc`** 去 frontmatter 后写入 **`./.codex/topics/*.md`**，与本文件、**`./.codex/skills/`** 一起构成 Flow2Spec 的可执行依据。**当前包模板**对应文件（运行时路径均在仓库根下 `.codex/topics/`）：
