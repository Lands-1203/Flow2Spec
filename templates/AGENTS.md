...
11. **普通问答源码补答收口**：仅当本轮是**普通问答 / 排查 / 解释**，且**未进入** `f2s-*` 技能、`implement-tech-design`、`f2s-git-commit` 或其他已有后续流程时适用。读完首个业务源码文件即视为 `sourceFallbackUsed=true`；若最终答案引用源码事实，只在需要补充知识库时输出 `> **知识库补充建议**` 引用块，且块内只放一条命令：KB 未覆盖用 `f2s-kb-add <模块路径>`，KB 不够细 / 与代码不一致用 `f2s-kb-sync <topicId>：<本轮新增或修正事实摘要>`。不需要补充时静默，不输出 case=4、已读 KB、覆盖对照或解释。若下钻前已说明「仍缺 X / KB 未覆盖 X / topic 未覆盖 X」，答后强制建议 sync，不得静默。只提示，不自动执行 `f2s-kb-add` / `f2s-kb-sync`。
