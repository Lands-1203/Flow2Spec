---
name: /生成后端技术文档
id: gen-backend-tech-doc
category: 工作流
description: 根据澄清后的需求（及可选约束）基于项目知识库/Skills/Rules 生成后端技术文档
---
> **「配置根」**：当前 agent 对应的 AI 工具配置目录（`flow2spec init` 写入，常见 **`.cursor/`**、**`.claude/`**、**`.codex/`**）。下文 **`配置根/...`** 指该目录下的相对路径。

# 根据需求生成后端技术文档

用户会在本命令后附带**已澄清的需求**（或需求摘要、PRD 路径），并可选择附带**需求条件**（如范围限定、必须/禁止使用的技术、优先级等）。你需要**基于当前项目的知识库、配置根/skills、配置根/rules** 理解项目约定与能力，针对该需求输出一份**后端技术文档**。

**用途**：本命令产出的技术方案**供后续代码实现使用**，开发按该文档实现功能。不用于生成 Rules/Skills。
...
生成文档时，**按 `配置根/template/后端技术模版.md` 中的章节顺序与书写要求**输出；与需求无关的整节可省略。执行本命令时请**先读取该模版**作为结构与表述参考。
