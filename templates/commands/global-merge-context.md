...
- **`/修正实现规则`（global-fix）**：用户已指明问题点后的**定向修正**与文档/规则同步。
- **本命令**：合并产生的**批量冲突**，侧重**编辑器上下文与说明文档**同**实现侧**分离处理。
