...
- **方式 A：传技术方案路径**  
  第一个参数为**技术方案文档路径**（如 `docs/xxx技术方案.md`、`.cursor/docs/xxx.md`）。  
...
- 若参数中**包含** `/`、`\`、`.md`、`docs`、`.cursor` 等路径特征，或明显为文件路径 → 视为**技术方案路径**。
...
- 读取并执行 `.cursor/commands/spec2context-md.md` 中的逻辑（或项目内等价命令说明）。
- **输入**：步骤 1 确定的**技术方案路径**（可能是 `docs/xxx.md` 或 `openspec/changes/<name>/design.md` 等）。
- **输出**：得到符合《终稿模版》的文档，默认写入 **`.cursor/docs/<方案名>.md`**（方案名由一级标题或文件名推断，做合法文件名处理）。
...
- 读取并执行 `.cursor/commands/generateProjectContext.md` 中的逻辑。
- **输入**：步骤 2 产出的 **`.cursor/docs/<方案名>.md`**。
- **输出**：更新/生成 `.cursor/rules/`、`.cursor/skills/`、`.cursor/docs-index.md` 及 main 等，即**提交到全局 Rules、Skills 与文档索引**（本命令的核心作用之一）。

### 步骤 4：若需要同步规范，执行 /opsx-sync

- **仅当**步骤 1 确定「同步规范 = 是」时执行。
- 读取并执行 `.cursor/commands/opsx-sync.md` 中的逻辑。
...
- 每步完成后可简要说明该步结果（如：已生成 `.cursor/docs/<方案名>.md`；**已提交到全局 Rules、Skills、索引**；若执行了同步规范则说明更新了哪些 capability）。
...
- 路径均相对于**项目根**；顺序为：转功能概述 → **提交到全局 Rules/Skills** → 可选**同步规范**，不可颠倒。
