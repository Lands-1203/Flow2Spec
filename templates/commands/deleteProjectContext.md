...
用户会在本命令后附带**一个参数**：与 `/generateProjectContext` 一致，为**本地路径**（如 `.cursor/docs/技术方案设计.md`）或**文档路径中可匹配的片段**（如 `技术方案设计.md`）。文档通常在项目根下的 **`.cursor/docs/`** 或 **`docs/`** 目录（以项目约定为准）。请按以下步骤执行，**直接删除**匹配到的产物并更新索引与 main。

---

## 步骤 1：解析用户输入路径

- 用户给出的是**本地路径**（如 `.cursor/docs/技术方案设计.md`）或**文件名/片段**（如 `技术方案设计.md`）。
- 用于在 docs-index 中匹配「文档路径」列：若文档路径列是链接 `[.cursor/docs/xxx.md](docs/xxx.md)` 或纯路径 `.cursor/docs/xxx.md`，则匹配路径或文件名包含用户输入即可。

---

## 步骤 2：读取 docs-index 并匹配行

- **路径**：`.cursor/docs-index.md`（相对于当前项目根目录）。
...
1. **Rules 列**：取该列内容，按顿号、逗号或空格拆成多个路径；每个路径相对于项目根（如 `.cursor/rules/<主题>-context.mdc`）。**删除**这些文件（若文件不存在则忽略）。
2. **Skills 列**：取该列内容，同样拆成多个路径（如 `.cursor/skills/<主题>-context/SKILL.md`）。**删除**这些文件；删除后**必须**检查该 Skill 所在的父目录，若目录已空（无其它文件），**必须删除该空目录**，避免留下空文件夹。
...
- **写回**：将修改后的内容**写回** `.cursor/docs-index.md`，确保文件已更新。

---

## 步骤 5：更新 main.mdc，删除与该文档相关的公共描述

- **路径**：`.cursor/rules/main.mdc`。
...
- **写回**：将修改后的 main.mdc 写回 `.cursor/rules/main.mdc`。
