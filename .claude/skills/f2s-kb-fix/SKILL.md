...
3. 同步知识库（默认执行）：
   ...
   - 路由清单：若路由受影响则最小更新
   - **创作侧准则**：本步若新增 / 修改 topic 或 `topicDependencies`，须先 Read `rules/f2s-topic-authoring.*` 全文（**Cursor/Claude**：`rules/f2s-topic-authoring.mdc`；**Codex**：`.codex/topics/f2s-topic-authoring.md`），再落盘。
4. 输出摘要（代码改动 + 知识库改动）。
