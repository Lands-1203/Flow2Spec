...
Claude 项目执行 `flow2spec init claude` 后会把更新检测拆成两段：`SessionStart` 执行 `f2s-update-check.js` 做完整检测并写 `.Knowledge/update-check.json`，`UserPromptSubmit` 执行 `f2s-update-notice.js` 只读缓存并兜底首轮用户消息提示；Cursor / Codex 仍走会话启动 hook。脚本提示格式为：“当前项目「<项目名>」的知识库版本是 v<当前版本>，低于最新包版本 v<最新版本>。如需对齐模板与路由，可以执行 f2s-kb-upgrade skill。”若当天缓存已确认需要升级，新会话会继续提醒；升级成功后 `f2s-kb-upgrade` 会清理 `.Knowledge/update-check.json`，避免旧提示残留。
