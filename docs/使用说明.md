...
Claude / Cursor / Codex 的启动 hook 只负责把升级提示注入上下文；Agent 通常不会主动开口，用户发出本会话第一条消息后才会在回复中提醒执行 `f2s-kb-upgrade`。若当天缓存已确认需要升级，新会话会继续提醒；升级成功后 `f2s-kb-upgrade` 会清理 `.Knowledge/update-check.json`，避免旧提示残留。
