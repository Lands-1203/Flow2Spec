...
Flow2Spec 通过五层机制提高覆盖率：① 路由层 matcher 找主 topic；② `topicDependencies` 展开依赖 topic；③ `f2s-kb-sync` 先给出更新大纲让用户确认；④ 普通问答下钻源码后提示补充；⑤ `f2s-git-commit` 提交前做知识覆盖检查。
...
两个典型场景说明这五层如何生效：一是防止意图误判（用户还在澄清阶段，Agent 不能跳去实现）；二是防止源码答完后知识不反哺（必须判断是否需要 `f2s-kb-add` 或 `f2s-kb-sync`，不允许静默跳过）。

Flow2Spec 的目标不是完全消除遗忘，而是让"绕过规则"在每个阶段都更难发生。

**Q：单个主题文件过大怎么办？**

topic 的定位是"路由摘要"，记触发词、边界、关键约束和下一步指针，不承载所有细节。大功能拆成多个 focused topic，再用 `topicDependencies` 串起来。Agent 先读概览判断相关性，再按需下钻具体子主题。

**Q：如果知识库没有覆盖当前模块怎么办？**

如果 Agent 通过源码找到了答案但知识库没覆盖，它应该提示 `f2s-kb-add <模块路径>`（整体入库）；如果模块已入库只是 topic 没写全，则提示 `f2s-kb-sync <主题说明>`（同步增量知识）。每次"从源码查答案"都是一次知识库补强的机会。
