...
async function run(cwd, agentIds, options = {}) {
  ...
  for (const id of ids) {
    ...
    // Cursor / Codex：复制 update-check hook 脚本（规则层执行用）
    if (id === "cursor" || id === "codex") {
      const agentRoot = AGENTS[id].root;
      const hooksDir = path.join(cwd, agentRoot, "hooks");
      const src = path.join(templatesDir, "hooks", "f2s-update-check.js");
      if (fs.existsSync(src)) {
        if (!fs.existsSync(hooksDir)) fs.mkdirSync(hooksDir, { recursive: true });
        fs.copyFileSync(src, path.join(hooksDir, "f2s-update-check.js"));
      }
    }
  }
  return {
