...
function buildMergedRouting(templateRouting, existingRouting, pkgVersion) {
  ...
  // templateRevision：标记包模板的「主题层」修订号。模板优先（每次 init 都以包为准透写），
  // 供 f2s-kb-upgrade 在 init 后比对「包带过来的 revision」与「项目侧上次 init 留下的 revision」，
  // 一致即「主题层未变」可走快速路径，不一致才跑完整流程。模板未声明该字段时不写入。
