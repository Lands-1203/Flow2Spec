...
function buildMergedRouting(templateRouting, existingRouting, pkgVersion, isFirstInit = false) {
  ...
  // templateRevision：表示「本项目已基线对齐到的包模板修订号」（由 f2s-kb-upgrade 完整流程跑完 3a/3b 后写入）。
  // init 行为：
  //   - 首次初始化（项目侧 manifest-routing.json 此前不存在）：按模板写入，等同首次落地即视为已对齐。
  //   - 已存在 manifest-routing.json：不写、不覆盖项目侧值（即便项目侧为 null 或缺失）；
  //     由 f2s-kb-upgrade 在完整流程末尾依据「包侧 templateRevision」改写。
  // 这样 f2s-kb-upgrade 的 projectRev / pkgRev 才有清晰语义：
  //   - projectRev = 项目侧 manifest-routing.json.templateRevision（本项目已对齐版本）
  //   - pkgRev     = 包侧 templates/<locale>/knowledge/manifest-routing.json.templateRevision（最新包版本）
  if (isFirstInit) {
    if (Object.prototype.hasOwnProperty.call(templateRouting, "templateRevision")) {
      knownMerged.templateRevision = templateRouting.templateRevision;
    }
  } else if (
    Object.prototype.hasOwnProperty.call(existingRouting, "templateRevision")
  ) {
    knownMerged.templateRevision = existingRouting.templateRevision;
  }
