...
function mergeUpdateCheckHook(existing) {
  ...
  const beforeSessionStart = JSON.stringify(next.hooks.SessionStart || []);
  next.hooks.SessionStart = removeHookCommand(next.hooks.SessionStart, 'f2s-update-check');
  const removedFromSessionStart = beforeSessionStart !== JSON.stringify(next.hooks.SessionStart || []);

  if (hasHookCommand(next.hooks.UserPromptSubmit, 'f2s-update-check')) {
    return { settings: next, changed: removedFromSessionStart };
  }
