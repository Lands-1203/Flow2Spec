...
/**
 * 将 f2s UserPromptSubmit（版本检查）hook 合并进 settings。
...
function mergeUpdateCheckHook(existing) {
  ...
  if (hasHookCommand(next.hooks.UserPromptSubmit, 'f2s-update-check')) {
    return { settings: next, changed: false };
  }
...
function copyHookScript(claudeRoot, templatesDir, scriptName) {
  ...
  fs.copyFileSync(src, path.join(hooksDir, scriptName));
  return { written: true };
}

/**
 * 主入口：为 claude agent 配置 f2s hooks（PreToolUse config-inject + UserPromptSubmit update-check）。
 * @param {string} cwd
...
function writeClaudeAgentHooks(cwd, templatesDir) {
  ...
  const r2 = mergeUpdateCheckHook(settings);
  if (r2.changed) { settings = r2.settings; changed = true; }

  if (changed) writeSettings(claudeRoot, settings);

  return { hookScriptResult, updateCheckResult, settingsChanged: changed };
}

module.exports = { writeClaudeAgentHooks, mergeF2sHook, mergeUpdateCheckHook };
