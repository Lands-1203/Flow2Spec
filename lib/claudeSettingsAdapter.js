...
const HOOK_COMMAND_UPDATE_NOTICE = 'node .claude/hooks/f2s-update-notice.js';
 ...
 * - SessionStart：执行完整检测，写入 .Knowledge/update-check.json
 * - UserPromptSubmit：只读缓存并注入首条用户消息提示，不查 npm
 * @param {object} existing
 * @returns {{ settings, changed }}
 */
function mergeUpdateCheckHook(existing) {
  ...
  if (!next.hooks) next.hooks = {};
  if (!next.hooks.SessionStart) next.hooks.SessionStart = [];
  if (!next.hooks.UserPromptSubmit) next.hooks.UserPromptSubmit = [];

  const beforePrompt = JSON.stringify(next.hooks.UserPromptSubmit || []);
  next.hooks.UserPromptSubmit = removeHookCommand(next.hooks.UserPromptSubmit, 'f2s-update-check');
  const removedOldPromptCheck = beforePrompt !== JSON.stringify(next.hooks.UserPromptSubmit || []);

  let changed = removedOldPromptCheck;
  ...
  if (!hasHookCommand(next.hooks.UserPromptSubmit, 'f2s-update-notice')) {
    next.hooks.UserPromptSubmit.push({
      hooks: [{ type: 'command', command: HOOK_COMMAND_UPDATE_NOTICE }],
    });
    changed = true;
  }
...
function writeClaudeAgentHooks(cwd, templatesDir) {
  ...
  const updateNoticeResult = copyHookScript(claudeRoot, templatesDir, 'f2s-update-notice.js');
  ...
  return { hookScriptResult, configSessionResult, updateCheckResult, updateNoticeResult, settingsChanged: changed };
}
